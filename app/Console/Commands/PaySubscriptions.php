...
class PaySubscriptions extends Command
{
                        ...
                        foreach ($subscriptions_orders_item as $item) {
                            ...
                            if ($response['status'] == "success") {
                                if ($response['response']->details[0]->status != 'AUTHORIZED') {
                                    ...
                                    $this->handleSubscriptionsPayV2($item, $customer, $order, $response['response']->details[0]->authorization_code);
                                }
                            } else {
    ...
    private function handleSubscriptionsPayV2($item, $customer, $order, $authorization_code)
    {
        $order->discount = 0;
        $order->save();
        $subtotal = 0;
        ...
        $order->customer_id = $item->order->customer_id;

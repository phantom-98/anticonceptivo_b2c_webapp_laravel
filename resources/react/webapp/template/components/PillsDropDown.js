...
const PillsDropDown = ({
    ...
    const categoryUrl = PUBLIC_ROUTES.SHOP_FILTER.path.replace(
        ":category?",
        categorySlug
    );

    // need to split laboratories in 2 columns if there are more than 5 laboratories and add them in array
    let laboratories1 = laboratories.slice(
        0,
        Math.round(laboratories.length / 2)
    );
    let laboratories2 = laboratories.slice(
        Math.round(laboratories.length / 2),
        laboratories.length
    );
            ...
            <div className="row mt-1">
                <div className="col-4">
                    ...
                    {formats.map((format) => {
                        ...
                        let formatUrl = categoryUrl.replace(
                            ":type?",
                            "formato"
                        );
                        ...
                        return (
                            <Link
                                ...
                                key={formatKey}
                            >
                                <span className="font-poppins py-1 font-12 text-black my-auto d-block">
                                    {format} pastillas
                                </span>
                            </Link>
                        );
                    ...
                    >
                        <div className="pills-sub-header-title mt-3">
                            SUSCRIPCIÓN
                        </div>
                    </Link>
                    ...
                    {subscriptions.map((subscription) => {
                        ...
                        let subscriptionUrl = categoryUrl.replace(
                            ":type?",
                            "suscripcion"
                        );
                            ...
                            return (
                                <div className="col-6">
                                    {laboratories.map((laboratory) => {
                                        ...
                                        let laboratoryUrl = categoryUrl.replace(
                                            ":type?",
                                            "laboratorio"
                                        );
                                        laboratoryUrl = laboratoryUrl.replace(
                                            ":filter?",
                                            laboratory.name
                                                .replace(" ", "-")
                                                .toLowerCase()

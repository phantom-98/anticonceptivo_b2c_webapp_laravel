    ...
    <div class="mensaje">
        ...
        <div style="margin-bottom: 20px;">
            <table class="table" style="margin-bottom: 20px;">
                ...
                <tr>
                    <td class=" w-25">${{ number_format($subscription_order->subtotal, 0, ',','.')}}</td>
                    @if($order->dispatch == -1)

                        <td class=" w-25">Por pagar</td>

                    @else
                        <td class=" w-25">${{ number_format($subscription_order->dispatch, 0, ',','.')}}</td>



                    @endif
                    {{-- <td class=" w-25">${{ number_format($subscription_order->discount, 0, ',','.')}}</td> --}}
                    {{-- <td class=" w-25">${{ number_format($order->tips, 0, ',','.')}}</td> --}}
                    <td class=" w-25">${{ number_format($subscription_order->total, 0, ',','.')}}</td>
                </tr>
                </tbody>
            </table>
        </div>
        <div style="margin-bottom: 20px;">
            <table class="table">
                ...
                <tr>
                    ...
                    <td class="bold w-25" style="text-align: right;"><b> ${{ number_format($subscription_order->order_item->total, 0, ',','.')}}</b></td>
                </tr>
                </tbody>
            </table>
        </div>

            ...
            <div class="col-md-10 col-md-offset-1">
                ...
                <div class="panel">
                    <div class="panel-heading">
                        <h3 class="panel-title">Crear codigo de descuento</h3>
                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group ">
                                            ...
                                            <select class="form-control"
                                                    name="is_forever"
                                                    id="is_forever">
                                                <option value="0" {{ old("is_forever") == 0 ? "selected" : ""}}>Fecha de termino</option>
                                                <option value="1" {{ old("is_forever") ? "selected" : ""}}>Para siempre</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group ">
                                            ...
                                            <select class="form-control"
                                                    name="is_percentage"
                                                    id="is_percentage">
                                                <option value="0" {{ old("is_percentage") == 0 ? "selected" : ""}}>Valor</option>
                                                <option value="1" {{ old("is_percentage") ? "selected" : ""}}>Porcentaje</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-3"  id="idContainerDeadLine" {{ old("is_forever") ? "hidden" : ""}} >
                                        <div class="form-group {{ $errors->has('expiration_date') ? 'has-error':'' }}">
                                            ...
                                            <input type="date"
                                                ...
                                                min="{{ date('Y-m-d') }}"
                                                value="{{ old('expiration_date') ?? date('Y-m-d') }}"
                                        ...
                                        <div class="form-group {{ $errors->has('customer_id') ? 'has-error':'' }}">
                                            ...
                                            <select class="form-control"
                                                ...
                                                @foreach( $customers as $item)
                                                    <option value="{{ $item->id }}">{{ $item->first_name }} {{ $item->last_name }} | {{ $item->rut }}</option>
                                                @endforeach
                                            </select>
...
@section('scripts')
    ...
    <script>
        ...
        $('#is_percentage').change(function () {
            if($(this).val()==0){
                $('#discount').attr("placeholder", "Ej: 1000");
            }else{
                $('#discount').attr("placeholder", "Ej: 10");
            }
        });
